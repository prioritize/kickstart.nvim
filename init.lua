...
require('lazy').setup({
  ...
  },
  {
    ...
    lazy = false,
    opts = {
      -- enable just what you actually want
      picker = { enabled = true },
      explorer = { enabled = true }, -- optional, nice file tree / finder
      notifier = { enabled = true },
      quickfile = { enabled = true },
      scope = { enabled = true },
      scroll = { enabled = true },
      words = { enabled = true }, -- auto-show LSP refs in code
      -- if you want snacks to implement `vim.ui.select` for code actions:
      input = { enabled = true },
    },
    keys = {
      {
    ...
    config = function()
      ...
      local servers = {
        ...
        gopls = {
          settings = {
            gopls = {
              -- gopls returns no inlay hints unless they are enabled here;
              -- toggle them per-buffer with <leader>th
              hints = {
      ...
      -- mason-lspconfig v2 dropped the `handlers` option and enables installed
      -- servers itself, so per-server config is registered through
      -- `vim.lsp.config()` instead. Each table above is merged over the
      -- defaults nvim-lspconfig ships for that server.
  ...
  { -- Autoformat
    ...
    opts = {
      ...
      formatters_by_ft = {
        ...
        c = { 'clang-format' },
        cpp = { 'clang-format' },
        -- goimports formats *and* fixes the import block; gofumpt is a
        -- stricter superset of gofmt. Both need the Go toolchain installed.
...
-- Snacks picker as Telescope replacement
...
-- buffer-local fuzzy search (Kickstart’s `/` mapping)
vim.keymap.set('n', '/', function()
  picker.lines()
end, { desc = '[/] Fuzzily search in current buffer' })

-- live grep in open buffers (Kickstart’s `s/`)
vim.keymap.set('n', 's/', function()
  picker.grep_buffers()
end, { desc = '[S]earch [/] in Open Files' })

-- search your config (Kickstart’s `sn`)
